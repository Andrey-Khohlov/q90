...
-- 3) Для НФБК удалены внутренне производные атрибуты в coffees:
--    price_250g и price_1Kg определяются из (price, weight), а этот детерминант не является ключом.
--    weighted_rate обычно вычисляется из avg_rating и ratings_count, поэтому также исключён.
...
-- Справочник статусов (можно заменить CHECK-ограничением)
CREATE TABLE moderation_statuses (
    status_id VARCHAR(20) PRIMARY KEY,
);

INSERT INTO moderation_statuses VALUES 
    ('pending'),
    ('approved'),
    ('rejected');

-- Справочник причин жалоб (можно заменить CHECK-ограничением)
...
CREATE TABLE moderations (
    ...
    target_type VARCHAR(50) NOT NULL,   -- 'messages', 'reviews', 'coffee', 'location', 'organization', 'farm_detail', 'roaster_detail', 'variety', 'green_bean', 'grinder', 'brewer'
    target_id BIGINT NOT NULL,
    complainer_id BIGINT REFERENCES users(user_id),  -- должно быть объявлено без NOT NULL (что уже сделано), так как модерация может инициироваться системой.
    reason VARCHAR(50) NOT NULL REFERENCES reasons(reason_id),  -- 'spam', 'offensive', 'incorrect'
    description TEXT,
    status VARCHAR(20) NOT NULL DEFAULT 'pending' REFERENCES moderation_statuses(status_id),
    moderated_by BIGINT REFERENCES users(user_id),
...
COMMENT ON TABLE moderations IS 'Модерация различных объектов';

CREATE INDEX idx_moderations_target ON moderations(target_type, target_id);
