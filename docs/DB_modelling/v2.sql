...
CREATE TABLE organizations (
    ...
    org_types TEXT[] NOT NULL DEFAULT '{}', -- массив типов: 'farm', 'exporter', 'importer', 'roaster'
...
CREATE INDEX idx_organizations_types ON organizations USING GIN (org_types);
...
-- 3. ПОЛЬЗОВАТЕЛИ (без изменений, но ссылаются на locations)
CREATE TABLE users (
...
-- (Опционально: можно создать триггеры для обновления счетчиков followers в связанных таблицах,
-- но в рамках DDL это не включено.)

-- 10. ИНДЕКСЫ для новых таблиц (уже созданы выше, остальные перенесены)

-- 11. КОММЕНТАРИИ К ТАБЛИЦАМ (обновлены для изменённых)
COMMENT ON TABLE locations IS 'Географические объекты (части света, страны, регионы, местности, города)';
COMMENT ON TABLE organizations IS 'Организации-участники рынка (фермы, экспортёры, импортёры, обжарщики)';
COMMENT ON TABLE farm_details IS 'Детальная информация о фермах';
COMMENT ON TABLE roaster_details IS 'Детальная информация об обжарщиках';
...
-- Остальные комментарии можно перенести из исходного скрипта, но здесь они опущены для краткости.
